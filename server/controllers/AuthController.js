...
const calendarStatus = require('../helpers/types/status');
...
exports.Register = async function (req, resp) {
    try {
        ...
        if (dbResp.error)  
            throw new Error(`Error while adding main calendar to data base: ${dbResp.error_message}`);

        dbResp = await new usersCalendarsQ()
            .New()
            .Insert(
            {
                user_id: dbResp.author,
